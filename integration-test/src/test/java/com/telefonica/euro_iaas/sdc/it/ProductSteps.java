...
public class ProductSteps {
	...
	@When("^I add product \"([^\"]*)\" \"([^\"]*)\" \\(\"([^\"]*)\"\\)$")
	public void addProductToCatalog(String productName, String version,
			String description) throws Exception {
		ProductUtils manager = new ProductUtils();
		addedProduct = manager.add(productName, version, description, "",
	...
	public void getProduct(String productName, String version) throws Exception {
		...
		System.out.println("existed product: "
				+ existedProduct.getProduct().getName() + "version: "
				+ existedProduct.getVersion());
	}

	@When("^I update \"([^\"]*)\" \"([^\"]*)\" \\(\"([^\"]*)\"\\)$")
	public void updateProductToCatalog(String productName, String version,
			String description) throws Exception {
		ProductUtils manager = new ProductUtils();
		updatedProduct = manager.update(productName, version, description, "",
				attributes, supportedOS, transitableReleases);
	}
	...
	public void assertUpdatedProduct(String productName, String version) {
		...
		Assert.assertEquals(updatedProduct.getProduct().getName(),
				existedProduct.getProduct().getName());
		Assert.assertEquals(updatedProduct.getVersion(), existedProduct
				.getVersion());
		Assert.assertEquals(updatedProduct.getReleaseNotes(), existedProduct
				.getReleaseNotes());
		Assert.assertEquals(updatedProduct.getAttributes(), existedProduct
				.getAttributes());
		Assert.assertEquals(updatedProduct.getSupportedOOSS(), existedProduct
				.getSupportedOOSS());
		Assert.assertNotSame(updatedProduct.getProduct().getDescription(),
				existedProduct.getProduct().getDescription());
	}

	// DELETE PRODUCT
	@Given("^a product \"([^\"]*)\" \"([^\"]*)\" present in the Catalog$")
	public void getProductFromCatalog(String productName, String version)
			throws Exception {
		System.out.println("product: " + "version: " + version);
		ProductUtils manager = new ProductUtils();
		existedProduct = manager.load(productName, version);
	...
	public void assertDeletedProduct(String productName, String version)
			throws ResourceNotFoundException {
		// ProductUtils manager = new ProductUtils();
	...
	// DELETE COOKBOOK
	/*
	 * @Given("^default attributes  Product Releases:$") public void
	 * getProductReleasesAttributes(Table table) { attributes = new
	 * ArrayList<Attribute>(); for (List<String> row: table.rows()) {
	 * attributes.add(new Attribute(row.get(0), row.get(1), row.get(2))); } }
	 */
	...
	public void deleteProductReleaseCookbook(String productName,
