...
public class ApplicationInstanceSteps {
	...
	public void addApplicationToCatalog(String applicationName, String version)
			throws Exception {
	...
	public void iInstallTheApplicatioOnProducts(String applicationName,
			String version, Table table) throws Exception {
		/*
		 * List<ReleaseDto> products = new ArrayList<ReleaseDto>(); for
		 * (List<String> product : table.rows()) { products.add(new
		 * ReleaseDto(product.get(0), product.get(1), null)); }
		 * ApplicationInstanceUtils manager = new ApplicationInstanceUtils();
		 * String vdc = getProperty(VDC); String ip = getProperty(VM);
		 * createdApplication = manager.installIfNotInstalled( applicationName,
		 * version, ip, vdc, products);
		 */
	}
	...
	public void iGetTheApplicationInstalledInVm(String applicationName,
		...
		ApplicationInstanceUtils manager = new ApplicationInstanceUtils();
		Assert.assertEquals(createdApplication, manager.load(vdc,
				createdApplication.getName()));
		Assert.assertEquals(createdApplication.getApplication().getVersion(),
				(version));
		Assert.assertEquals(createdApplication.getApplication()
				.getApplication().getName(), (applicationName));
	...
	@When("^I configure the application \"([^\"]*)\" \"([^\"]*)\""
			+ " with \"([^\"]*)\" \"([^\"]*)\"$")
	public void iConfigureTheApplication(String applicationName,
			String version, String key, String value) throws Exception {
		ApplicationInstanceUtils manager = new ApplicationInstanceUtils();
		ArrayList<Attribute> attributes = new ArrayList<Attribute>();
		attributes.add(new Attribute(key, value));
		manager.configure(createdApplication.getVdc(), createdApplication
				.getName(), attributes);
	}

	@Then("^I get the application \"([^\"]*)\" \"([^\"]*)\" configured with \"([^\"]*)\" \"([^\"]*)\"$")
	@Pending
	public void iGetTheApplicationConfigured(String applicationName,
			String version, String key, String value) throws Exception {
	...
	public void updateApplication(String appName, String newVersion)
			throws Exception {
		ApplicationInstanceUtils manager = new ApplicationInstanceUtils();
		manager.upgrade(createdApplication.getVdc(), createdApplication
				.getName(), newVersion);
	}
	...
	public void uninstallApplication(String applicationName, String version)
			throws Exception {
		ApplicationInstanceUtils manager = new ApplicationInstanceUtils();
		manager.uninstall(createdApplication.getVdc(), createdApplication
				.getName());
	}
