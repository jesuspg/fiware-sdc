...
public class ProductInstanceUtils {
	 ...
	 */
	public ProductInstance install(String productName, String version,
			String ip, String vdc) throws MaxTimeWaitingExceedException,
			InvalidExecutionException {
		service = client.getProductInstanceSyncService(getProperty(BASE_URL),
				getProperty(MIME_TYPE));
		/*
		 * ProductInstanceDto instance = new ProductInstanceDto( new
		 * ProductReleaseDto( productName, null, version, null, null, null,
		 * null), new VM(ip));
		 */
		ProductInstanceDto instance = new ProductInstanceDto(new ReleaseDto(
				productName, version, "product"), new VM(ip));
	 ...
	 *             if the installation takes more time than expected
	 * @throws InvalidExecutionException
	 *             if the product can not be configured
	 */
	public ProductInstance configure(String vdc, String name,
			List<Attribute> attributes) throws MaxTimeWaitingExceedException,
			InvalidExecutionException {
		service = client.getProductInstanceSyncService(getProperty(BASE_URL),
				getProperty(MIME_TYPE));
	 ...
	 *             if the product can not be configured
	 */
	public ProductInstance upgrade(String vdc, String name, String newVersion)
	 ...
	 *             if the product can not be configured
	 */
	public ProductInstance uninstall(String vdc, String name)
			throws MaxTimeWaitingExceedException, InvalidExecutionException {
		service = client.getProductInstanceSyncService(getProperty(BASE_URL),
				getProperty(MIME_TYPE));
	 ...
	 * @param vdc
	 *            defines the vdc where the products are installed (<i>not
	 *            nullable</i>).
	 * @return the products
	 */
	public List<ProductInstance> loadAll(String hostname, String domain,
		...
		service = client.getProductInstanceSyncService(getProperty(BASE_URL),
				getProperty(MIME_TYPE));
		return service.findAll(hostname, domain, ip, fqn, page, pageSize,
				orderBy, orderType, status, vdc, productName);
	 ...
	 *             if the product can not be configured
	 */
	public ProductInstance installIfNotInstalled(String productName,
			String version, String ip, String vdc)
			throws MaxTimeWaitingExceedException, InvalidExecutionException {
	 ...
	 * @throws InvalidExecutionException
	 */
	private ProductInstance uninstallIfNeed(ProductInstance instance,
			String version) throws MaxTimeWaitingExceedException,
			InvalidExecutionException {
		if (!instance.getProductRelease().getVersion().equals(version)
	...
	private boolean isInstalled(ProductInstance instance) {
		return (!instance.getStatus().equals(Status.ERROR) && !instance
				.getStatus().equals(Status.UNINSTALLED));
	}
