...
public class ApplicationInstanceUtils {
	 ...
	 */
	public ApplicationInstance install(String applicationName, String version,
			String ip, String vdc, EnvironmentInstanceDto environmentInstance)
			throws MaxTimeWaitingExceedException, InvalidExecutionException {
		service = client.getApplicationInstanceSyncService(
				getProperty(BASE_URL), getProperty(MIME_TYPE));
	 ...
	 *             if the application can not be configured
	 */
	public ApplicationInstance upgrade(String vdc, String name,
			String newVersion) throws MaxTimeWaitingExceedException,
			InvalidExecutionException {
		service = client.getApplicationInstanceSyncService(
				getProperty(BASE_URL), getProperty(MIME_TYPE));
	 ...
	 *             if the application can not be configured
	 */
	public ApplicationInstance installIfNotInstalled(String applicationName,
		...
		service = client.getApplicationInstanceSyncService(
				getProperty(BASE_URL), getProperty(MIME_TYPE));
		List<ApplicationInstance> instances = service.findAll(null, null, ip,
				null, null, null, null, null, null, vdc, applicationName);
		if (!instances.isEmpty()) {
			instance = instances.iterator().next();
			instance = uninstallIfNeed(instance, version);
		} else {
			instance = install(applicationName, version, ip, vdc,
					envronmentInstance);
		}
	 ...
	 * @throws InvalidExecutionException
	 */
	private ApplicationInstance uninstallIfNeed(ApplicationInstance instance,
