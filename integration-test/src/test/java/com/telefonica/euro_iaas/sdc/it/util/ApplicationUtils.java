...
public class ApplicationUtils {
    ...
    public ApplicationRelease load(String applicationName, String version) throws ResourceNotFoundException {
        service = client.getApplicationService(getProperty(BASE_URL), getProperty(MIME_TYPE));
     ...
     */
    public ApplicationRelease add(String applicationName, String version, String type, String description,
            String releaseNotes, List<Attribute> attributes, EnvironmentDto environment,
            List<ApplicationRelease> transitableReleases) {
        service = client.getApplicationService(getProperty(BASE_URL), getProperty(MIME_TYPE));

        ApplicationReleaseDto instance = new ApplicationReleaseDto(applicationName, description, type, version,
                releaseNotes, attributes, environment, transitableReleases);
        InputStream cookbook = this.getClass().getResourceAsStream(
                "/files/" + applicationName + version + "-cookbook.tar");
        InputStream binaries = this.getClass().getResourceAsStream("/files/" + applicationName + version + "-bin.tar");
     ...
     *             if the product can not be installed
     */
    public ApplicationRelease update(String applicationName, String version, String description, String type,
            String releaseNotes, List<Attribute> attributes, EnvironmentDto environment,
            List<ApplicationRelease> transitableReleases) {
        service = client.getApplicationService(getProperty(BASE_URL), getProperty(MIME_TYPE));
