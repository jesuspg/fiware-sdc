...
public class ProductInstanceSteps {
	...
	public void iInstallProductInVM(String productName, String version)
			throws Exception {
		String vdc = getProperty(VDC);
		String ip = getProperty(VM);

		ProductInstanceUtils manager = new ProductInstanceUtils();
		createdProduct = manager.installIfNotInstalled(productName, version,
				ip, vdc);
	}
	...
	public void iGetProductInstalledInVm(String productName, String version)
			throws Exception {

		String vdc = getProperty(VDC);

		ProductInstanceUtils manager = new ProductInstanceUtils();
		Assert.assertEquals(createdProduct, manager.load(vdc, createdProduct
				.getName()));
		Assert.assertEquals(createdProduct.getProductRelease().getVersion(),
				(version));
		Assert.assertEquals(createdProduct.getProductRelease().getProduct()
				.getName(), (productName));
	...
	public void iUninstallProductFromVdc(String productName, String version)
			throws Exception {
		ProductInstanceUtils manager = new ProductInstanceUtils();
		createdProduct = manager.uninstall(createdProduct.getVdc(),
	...
	@When("^I updrade to \"([^\"]*)\" \"([^\"]*)\"$")
	public void iUpdradeToNewProductVersion(String productName,
			String newVersion) throws Exception {
		String vdc = getProperty(VDC);
		ProductInstanceUtils manager = new ProductInstanceUtils();
		updatedProduct = manager.upgrade(vdc, createdProduct.getName(),
				newVersion);
	}

	@Then("^I get \"([^\"]*)\" upgraded from version \"([^\"]*)\" to \"([^\"]*)\" in VM$")
	public void iGetProductUpgradedInVm(String productName, String oldVersion,
			String newVersion) throws Exception {
		...
		Assert.assertEquals(newVersion, updatedProduct.getProductRelease()
				.getVersion());
	...
	public void iConfigureProductAtVdc(String key, String value)
		...
		createdProduct = manager.configure(createdProduct.getVdc(),
				createdProduct.getName(), attributes);
	}
