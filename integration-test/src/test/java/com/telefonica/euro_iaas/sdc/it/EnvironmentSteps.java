...
public class EnvironmentSteps {
    ...
    public void addEnvironmentToCatalog(String envName, String envDesc) throws Exception {
        EnvironmentUtils manager = new EnvironmentUtils();
        environmentInserted = manager.insert(envName, envDesc, productReleaseDtoInCatalog);
    ...
    public void getEnvironment(String envName, String envDesc) throws Exception {
        ...
        System.out.println("existed Environment: " + existedEnvironment.getName() + " wth description: "
                + existedEnvironment.getDescription());
    }

    @When("^I update \"([^\"]*)\" Environment with the new description \"([^\"]*)\"$")
    public void updateProductToCatalog(String envName, String envDescription) throws Exception {
        ...
        for (int i = 0; i < existedEnvironment.getProductReleases().size(); i++) {
            ProductReleaseDto productReleaseDto = new ProductReleaseDto();
            ProductRelease productRelease = existedEnvironment.getProductReleases().get(i);
            productReleaseDto.setPrivateAttributes(productRelease.getPrivateAttributes());
            productReleaseDto.setProductDescription(productRelease.getProduct().getDescription());
            productReleaseDto.setProductName(productRelease.getProduct().getName());
            ...
            productReleaseDto.setTransitableReleases(productRelease.getTransitableReleases());
        ...
        EnvironmentUtils manager = new EnvironmentUtils();
        updatedEnvironment = manager.update(existedEnvironment.getName(), existedEnvironment.getDescription(),
                productReleaseDtos);

        System.out.println("Updted Environment: " + updatedEnvironment.getName() + " wth description: "
    ...
    public void assertUpdatedEnvironment(String envName) {
        ...
        Assert.assertEquals(updatedEnvironment.getName(), existedEnvironment.getName());
        Assert.assertEquals(updatedEnvironment.getProductReleases(), existedEnvironment.getProductReleases());
        Assert.assertNotSame(updatedEnvironment.getDescription(), existedEnvironment.getDescription());
    }
    ...
    public void getProductFromCatalog(String envName) throws Exception {
        ...
        EnvironmentUtils manager = new EnvironmentUtils();
        existedEnvironment = manager.load(envName);
        System.out.println("existed environment: " + existedEnvironment.getName());
    }
    ...
    public void assertDeletedProduct(String envName) throws ResourceNotFoundException {
        EnvironmentUtils manager = new EnvironmentUtils();
