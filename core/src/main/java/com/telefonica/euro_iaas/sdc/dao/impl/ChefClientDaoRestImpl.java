...
public class ChefClientDaoRestImpl implements ChefClientDao {
	...
	public ChefClient chefClientfindByHostname(String hostname) 
			throws EntityNotFoundException, CanNotCallChefException {
		
		try {
			String path ="/clients";		
			
			Map<String, String> header = getHeaders("GET", path, "");
			WebResource webResource = client.resource(propertiesProvider
	...
	public void deleteChefClient(String chefClientName) throws CanNotCallChefException {
		try {
			String path = MessageFormat.format(propertiesProvider
					.getProperty(CHEF_SERVER_CLIENTS_PATH), chefClientName);
			...
			Builder wr = webResource.accept(MediaType.APPLICATION_JSON).type(
	 ...
	 */
	public ChefClient getChefClient(String chefClientName)
			throws CanNotCallChefException, EntityNotFoundException {
	...
	try {
		...
		if (stringChefClient == null)
			throw new EntityNotFoundException(ChefClient.class, null, "ChefClient " + chefClientName + " is not in the ChefServer");
	...
	private Map<String, String> getHeaders(String method, String path,
		...
		return digester.digest(method, path, payload, new Date(),
				propertiesProvider.getProperty(CHEF_CLIENT_ID),
