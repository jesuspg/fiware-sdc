 ...
 * 
 */
public class MixlibAuthenticationDigesterImpl implements
		MixlibAuthenticationDigester {
	...
	public Map<String, String> digest(String method, String path, String body,
		...
		try {
			DateFormat df = new SimpleDateFormat(propertiesProvider
					.getProperty(CHEF_DATE_FORMAT));
			df.setTimeZone(TimeZone.getTimeZone(propertiesProvider
					.getProperty(CHEF_TIME_ZONE)));
			...
			String digest = MessageFormat.format(BODY_TEMPLATE, method
					.toUpperCase(), getHash(path), getHash(body), df
					.format(timestamp), id);
			...
			while (!digest.isEmpty()) {
				...
				headers.put(AUTH_HEADER_PREFIX + iteration, digest.substring(0,
						length));
