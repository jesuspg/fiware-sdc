...
public class ApplicationInstanceDaoJpaImpl extends
		AbstractInstallableInstanceDaoJpaIml<ApplicationInstance, Long>
	...
	public List<ApplicationInstance> findByCriteria(
		...
		if (!StringUtils.isEmpty(criteria.getApplicationName())) {
			baseCriteria.createAlias("application", "rls").createAlias(
					"rls.application", "app");
			baseCriteria.add(Restrictions.eq("app.name", criteria
					.getApplicationName()));
		}
	...
	private List<ApplicationInstance> filterByProduct(
		...
		for (ApplicationInstance application : applications) {
			if (application.getEnvironmentInstance().getProductInstances()
					.contains(product)) {
