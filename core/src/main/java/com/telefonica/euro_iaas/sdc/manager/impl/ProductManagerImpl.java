...
public class ProductManagerImpl extends BaseInstallableManager implements
	...
	public ProductRelease insert(ProductRelease productRelease, File cookbook,
		...
		LOGGER.info("Inserting productRelease " + productRelease.getVersion() + " " +  productRelease.getProduct().getName());
		...
		releaseDto.setType(propertiesProvider
				.getProperty(WEBDAV_PRODUCT_BASEDIR));
	...
	public ProductRelease insert(ProductRelease productRelease) throws AlreadyExistsProductReleaseException, InvalidProductReleaseException {
		LOGGER.info("Inserting productRelease " + productRelease.getVersion() + " " +  productRelease.getProduct().getName());
		ProductRelease productReleaseOut = insertProductReleaseBBDD(productRelease);
	...
	// *********** PRIVATE METHODS ************//
	private ProductRelease insertProductReleaseBBDD(
			ProductRelease productRelease)
			throws AlreadyExistsProductReleaseException,
			InvalidProductReleaseException {

		LOGGER.info("Inserting insertProductReleaseBBDD " + productRelease.getVersion() + " " +  productRelease.getProduct().getName());
		Product product;
	...
	private boolean isLastProductRelease(ProductRelease productRelease)
		...
		List<ProductRelease> productReleases = productReleaseDao
	...
	private List<OS> insertProductReleaseLoadSO(ProductRelease productRelease)
		...
		for (int i = 0; i < productRelease.getSupportedOOSS().size(); i++) {
			try {
				os = osDao.load(productRelease.getSupportedOOSS().get(i)
						.getName());
	...
	private Product insertProductReleaseLoadProduct(
			ProductRelease productRelease)
			throws InvalidProductReleaseException {

		LOGGER.info("Inserting insertProductReleaseLoadProduct " + productRelease.getVersion() + " " +  productRelease.getProduct().getName());
		Product product;
	...
	private ProductRelease insertProductRelease(ProductRelease productRelease)
		...
		LOGGER.info("Inserting insertProductRelease " + productRelease.getVersion() + " " +  productRelease.getProduct().getName());
		ProductRelease productReleaseOut;
		// Insert ProductRelease if not in BBDD
		try {
			productReleaseOut = productReleaseDao.load(productRelease
					.getProduct(), productRelease.getVersion());
			...
			try {
				...
				String invalidEntityMessageLog = "The Product Release "
						+ productRelease.getProduct().getName()
						+ productRelease.getVersion()
						+ " is invalid. Please Insert a valid Product Release " + e1.getMessage();
	...
	private List<OS> updateProductReleaseLoadSO(ProductRelease productRelease)
		...
		/*
		 * OS os; List<OS> OSs = new ArrayList<OS>();
		 * 
		 * for (int i=0; i<productRelease.getSupportedOOSS().size(); i++) { try
		 * { os =
		 * osDao.load(productRelease.getSupportedOOSS().get(i).getName());
		 * LOGGER.log(Level.INFO,"OS " +
		 * productRelease.getSupportedOOSS().get(i).getName() + " LOADED");
		 * OSs.add(os);
		 * 
		 * } catch (EntityNotFoundException e) { String entityNotFoundMessageLog
		 * = "The OS " + productRelease.getSupportedOOSS().get(i).getName() +
		 * " has not been found in the System ";
		 * 
		 * LOGGER.log (Level.SEVERE, entityNotFoundMessageLog); throw new
		 * ProductReleaseNotFoundException ( entityNotFoundMessageLog, e); }
		 * 
		 * } return OSs;
		 */
	}
	...
	private Product updateProductReleaseLoadProduct(
			ProductRelease productRelease)
			throws ProductReleaseNotFoundException,
			InvalidProductReleaseException {
		...
		try {
			existedProduct = productDao.load(productRelease.getProduct()
					.getName());
			LOGGER.log(Level.INFO, "Product " + existedProduct.getName()
					+ " LOADED");

			if (productRelease.getProduct().getDescription() != null)
				existedProduct.setDescription(productRelease.getProduct()
						.getDescription());

			if (productRelease.getProduct().getAttributes() != null)
				existedProduct.setAttributes(productRelease.getProduct()
						.getAttributes());
			...
			throw new ProductReleaseNotFoundException(entityNotFoundMessageLog,
					e);
	...
	private ProductRelease updateProductRelease(ProductRelease productRelease)
		...
		try {
			...
			existedProductRelease = productReleaseDao.load(productRelease
					.getProduct(), productRelease.getVersion());
			...
			if (productRelease.getPrivateAttributes() != null)
				existedProductRelease.setPrivateAttributes(productRelease
						.getPrivateAttributes());
			if (productRelease.getReleaseNotes() != null)
				existedProductRelease.setReleaseNotes(productRelease
						.getReleaseNotes());
			if (productRelease.getSupportedOOSS() != null)
				existedProductRelease.setSupportedOOSS(productRelease
						.getSupportedOOSS());
			if (productRelease.getTransitableReleases() != null)
				existedProductRelease.setTransitableReleases(productRelease
			...
			String invalidEntityException = "The Product Release"
					+ productRelease.getProduct().getName() + " version "
