...
public class ProductInstanceDaoJpaImpl extends
		AbstractInstallableInstanceDaoJpaIml<ProductInstance, Long> implements
		ProductInstanceDao {
	...
	public ProductInstance load(String name) throws EntityNotFoundException {
		...
		/*
		 * } catch (Exception e) { try { return
		 * super.loadByField(ProductInstance.class, "name", name); } catch
	...
	public List<ProductInstance> findByCriteria(
		...
		if (criteria.getStatus() != null) {
			baseCriteria.add(Restrictions.eq(InstallableInstance.STATUS_FIELD,
					criteria.getStatus()));
		}
		if (criteria.getProductRelease() != null) {
			baseCriteria.add(Restrictions.eq(ProductInstance.PRODUCT_FIELD,
					criteria.getProductRelease()));
		}

		if (!StringUtils.isEmpty(criteria.getProductName())) {
			baseCriteria.createAlias("productRelease", "rls").createAlias(
					"rls.product", "prod");
	...
	@SuppressWarnings("unchecked")
	@Override
	public ProductInstance findByProductInstanceName(String productInstanceName)
		...
		/*
		 * SELECT * FROM productinstance LEFT JOIN artifact ON
		 * productinstance.id=artifact.productinstance_id;
		 */
		...
		Query query = (Query) entityManager
				.createQuery("select p from ProductInstance p  where p.name = '"
						+ productInstanceName + "'");
		...
		try {
			...
			String message = " No ProductInstance found in the database "
					+ "with name: " + productInstanceName;
			System.out.println(message);
			throw new EntityNotFoundException(ProductInstance.class, "name",
					productInstanceName);
		}
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<ProductInstance> findByHostname(String hostname)
			throws EntityNotFoundException {

		Query query = (Query) entityManager
				.createQuery("select p from InstallableInstance p  where p.vm.hostname = '"
						+ hostname + "'");
		...
		if (productInstances.isEmpty()) {
			String message = " No ProductInstancse found in the database "
					+ "installed on hostname: : " + hostname;
			System.out.println(message);
			throw new EntityNotFoundException(ProductInstance.class, "hostaname",
					hostname);
		}
