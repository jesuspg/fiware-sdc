 ...
 */
public class ApplicationInstanceValidatorImpl implements ApplicationInstanceValidator {
    ...
    @Override
    public void validateInstall(ApplicationInstance application) throws IncompatibleProductsException,
        ...
        for (ProductInstance product : application.getEnvironmentInstance().getProductInstances()) {
            if (!application.getApplication().getEnvironment().getProductReleases()
                    .contains(product.getProductRelease())) {
                invalidProducts.add(product);
            }
            if (product.getStatus().equals(Status.ERROR) || product.getStatus().equals(Status.UNINSTALLED)
        ...
        if (!invalidProducts.isEmpty()) {
            throw new IncompatibleProductsException(application, invalidProducts);
        }
     ...
     */
    @Override
    public void validateUninstall(ApplicationInstance application) throws FSMViolationException {
     ...
     */
    @Override
    public void validateConfigure(ApplicationInstance application) throws FSMViolationException {
     ...
     */
    @Override
    public void validateUpdate(ApplicationInstance application, ApplicationRelease newRelease)
        ...
        if (!application.getApplication().getTransitableReleases().contains(newRelease)) {
        ...
        for (ProductInstance product : application.getEnvironmentInstance().getProductInstances()) {
            if (!newRelease.getEnvironment().getProductReleases().contains(product.getProductRelease())) {
                incompatibleProducts.add(product);
            }
        }
        if (!incompatibleProducts.isEmpty()) {
            throw new IncompatibleProductsException(application, incompatibleProducts);
        }
    }
