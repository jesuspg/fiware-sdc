 ...
 */
public class ApplicationInstanceValidatorImpl
    implements ApplicationInstanceValidator {
    ...
    public void validateInstall(ApplicationInstance application)
        ...
        for (ProductInstance product : application.getEnvironmentInstance()
            ...
            if (product.getStatus().equals(Status.ERROR)
                ...
                uninstalledProducts.add(product.getProduct());
            }
        }
    ...
    public void validateUpdate(ApplicationInstance application,
            ApplicationRelease newRelease) throws IncompatibleProductsException,
            NotTransitableException, FSMViolationException{
        fsmValidator.validate(application, Status.CONFIGURING);
        //validate the transition between two versions
        if (!application.getApplication().getTransitableReleases()
                .contains(newRelease)) {
        ...
        if (!incompatibleProducts.isEmpty()) {
            throw new IncompatibleProductsException(
                    application, incompatibleProducts);
        }
    }

    ////////// I.O.C /////////
    /**
     * @param fsmValidator the fsmValidator to set
     */
