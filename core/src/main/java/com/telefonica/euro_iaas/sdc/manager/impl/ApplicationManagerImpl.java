 ...
 * 
 */
public class ApplicationManagerImpl extends BaseInstallableManager implements
		ApplicationManager {
	...
	public ApplicationRelease insert(ApplicationRelease applicationRelease,
		...
		releaseDto.setType(propertiesProvider
				.getProperty(WEBDAV_APPLICATION_BASEDIR));
	...
	private boolean isLastApplicationRelease(
			ApplicationRelease applicationRelease)
			throws ApplicationReleaseNotFoundException {
		boolean lastRelease = false;
		...
		List<ApplicationRelease> applicationReleases = applicationReleaseDao
	...
	private Environment loadEnvironment(ApplicationRelease applicationRelease)
		...
		for (int i = 0; i < applicationRelease.getEnvironment()
				.getProductReleases().size(); i++) {
			try {
				product = productDao.load(applicationRelease.getEnvironment()
						.getProductReleases().get(i).getProduct().getName());
				productNotFoundMessage = "Product "
						+ applicationRelease.getEnvironment()
								.getProductReleases().get(i).getProduct()
								.getName() + " LOADED";

				LOGGER.log(Level.INFO, productNotFoundMessage);
			} catch (EntityNotFoundException e) {
				throw new ProductReleaseNotFoundException(
						productNotFoundMessage, e);
			}
		...
		try {
			...
			LOGGER.log(Level.INFO, "Environment " + environment_name
					+ " LOADED");

		} catch (EntityNotFoundException e) {
			String environmentNotFoundMessage = "Environment "
					+ environment_name + " has not been Found in the system";

			LOGGER.log(Level.INFO, environmentNotFoundMessage);
			throw new EnvironmentNotFoundException(environmentNotFoundMessage,
					e);
		}
	 ...
	 * ApplicationRelease applicationRelease) throws
	 * ProductReleaseNotFoundException, InvalidProductReleaseException {
	 * 
	 * String productNotFoundMessage = null; ProductRelease productRelease;
	 * Product product; List<ProductRelease> productReleases = new
	 * ArrayList<ProductRelease>();
	 * 
	 * for (int i=0; i<applicationRelease.getSupportedProducts().size(); i++) {
	 * try { product = productDao.
	 * load(applicationRelease.getSupportedProducts()
	 * .get(i).getProduct().getName()); productNotFoundMessage = "Product " +
	 * applicationRelease.getSupportedProducts().get(i) .getProduct().getName()
	 * + " LOADED";
	 * 
	 * LOGGER.log(Level.INFO,productNotFoundMessage); } catch
	 * (EntityNotFoundException e) { throw new
	 * ProductReleaseNotFoundException(productNotFoundMessage,e); }
	 * 
	 * try { productRelease = productReleaseDao.load(product,
	 ...
	 * productReleases.add(productRelease);
	 * 
	 * } catch (EntityNotFoundException e) { String
	 * productReleaseNotFoundMessage = "Product Release " +
	 * applicationRelease.getSupportedProducts().get(i). getProduct().getName()
	 * + "-" + applicationRelease.getSupportedProducts().get(i).getVersion() +
	 * " has not been Found in the system";
	 * 
	 * LOGGER.log(Level.INFO,productReleaseNotFoundMessage); throw new
	 * ProductReleaseNotFoundException ( productReleaseNotFoundMessage,e); } }
	 * return productReleases;
	 * 
	 * }
	 * 
	 * private List<ProductRelease> updateApplicationReleaseLoadProductRelease
	 * (ApplicationRelease applicationRelease) throws
	 ...
	 * try { product = productDao.
	 * load(applicationRelease.getSupportedProducts()
	 * .get(i).getProduct().getName()); LOGGER.log(Level.INFO,"Product " +
	 * applicationRelease.getSupportedProducts().get(i).getProduct().getName() +
	 * " LOADED"); } catch (EntityNotFoundException e) { String
	 * productReleaseNotFoundMessageLog = "Product " +
	 * applicationRelease.getSupportedProducts().get(i).getProduct().getName() +
	 * " not found in the system"; throw new
	 * ProductReleaseNotFoundException(productReleaseNotFoundMessageLog,e); }
	 * try { productRelease = productReleaseDao. load(product,
	 * applicationRelease.getSupportedProducts().get(i).getVersion());
	 * 
	 * LOGGER.log(Level.INFO,"Product Release" + product.getName() + "-" +
	 * applicationRelease.getSupportedProducts().get(i).getVersion() +
	 * " LOADED"); productReleases.add(productRelease);
	 * 
	 * } catch (EntityNotFoundException e) { String
	 * productReleaseNotFoundMessageLog = "Product " +
	 * applicationRelease.getSupportedProducts().get(i).getProduct().getName() +
	...
	private Application insertApplicationReleaseLoadApplication(
		...
		try {
			application = applicationDao.load(applicationRelease
					.getApplication().getName());
			LOGGER.log(Level.INFO, "Application " + application.getName()
					+ " LOADED");
		} catch (EntityNotFoundException e) {
			try {
				application = applicationDao.create(applicationRelease
						.getApplication());
				LOGGER.log(Level.INFO, "Application " + application.getName()
						+ " CREATED");
	...
	private ApplicationRelease insertApplicationRelease(
		...
		try {
			applicationReleaseOut = applicationReleaseDao.load(
					applicationRelease.getApplication(), applicationRelease
							.getVersion());
	...
	private ApplicationRelease updateApplicationRelease(
		...
		try {
			...
			existedApplicationRelease = applicationReleaseDao.load(
					applicationRelease.getApplication(), applicationRelease
							.getVersion());
			...
			if (applicationRelease.getPrivateAttributes() != null)
				existedApplicationRelease
						.setPrivateAttributes(applicationRelease
								.getPrivateAttributes());
			if (applicationRelease.getReleaseNotes() != null)
				existedApplicationRelease.setReleaseNotes(applicationRelease
						.getReleaseNotes());
			if (applicationRelease.getEnvironment() != null)
				existedApplicationRelease.setEnvironment(applicationRelease
						.getEnvironment());
			if (applicationRelease.getTransitableReleases() != null)
				existedApplicationRelease
						.setTransitableReleases(applicationRelease
								.getTransitableReleases());

			applicationReleaseOut = applicationReleaseDao
					.update(existedApplicationRelease);
