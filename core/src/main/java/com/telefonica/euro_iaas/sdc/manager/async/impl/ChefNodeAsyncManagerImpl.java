...
public class ChefNodeAsyncManagerImpl implements ChefNodeAsyncManager {
	...
	private static Logger LOGGER = Logger
			.getLogger(ChefNodeAsyncManagerImpl.class.getName());
	...
	public void chefNodeDelete(String vdc, String chefNodename, Task task, String callback) {
		try {
			...
			LOGGER.info("Node  "
					+ chefNodename + " deleted from Chef Server successfully");
		} catch (NodeExecutionException e) {
			updateErrorTask(vdc, chefNodename,
					task,
					"The node "
							+ chefNodename
			...
			updateErrorTask(vdc, chefNodename, task, "The node "
					+ chefNodename
	...
	private void updateSuccessTask(Task task, String vdc, String chefNodename) {
		String piResource = MessageFormat.format(propertiesProvider
				.getProperty(CHEF_NODE_BASE_URL), vdc, chefNodename); // the product
	 ...
	 */	
	private void updateErrorTask(String vdc, String chefNodename, Task task,
			String message, Throwable t) {
		String piResource = MessageFormat.format(
				propertiesProvider.getProperty(CHEF_NODE_BASE_URL),
				vdc, chefNodename); // the product
		task.setResult(new TaskReference(piResource));
