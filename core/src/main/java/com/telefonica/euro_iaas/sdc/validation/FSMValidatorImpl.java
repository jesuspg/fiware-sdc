...
public class FSMValidatorImpl implements FSMValidator {
	...
	private final static List<Status> INSTALLING_STATES = Arrays.asList(
			Status.INSTALLED, Status.ERROR);
	private final static List<Status> INSTALLED_STATES = Arrays.asList(
			Status.UNINSTALLING, Status.UPGRADING, Status.CONFIGURING,
			Status.DEPLOYING_ARTEFACT, Status.ERROR);
	private final static List<Status> UNINSTALLING_STATES = Arrays.asList(
			Status.UNINSTALLED, Status.ERROR);
	private final static List<Status> UPGRADING_STATES = Arrays.asList(
			Status.INSTALLED, Status.ERROR);
	private final static List<Status> ARTIFACT_STATES = Arrays.asList(
			Status.ARTIFACT_DEPLOYED, Status.ERROR);

	private final static List<Status> CONFIGURING_STATES = Arrays.asList(
			Status.INSTALLED, Status.ERROR);
	private final static List<Status> UNINSTALLED_STATES = Arrays.asList(
			Status.ERROR, Status.INSTALLING);
	private final static List<Status> ERROR_STATES = Arrays
			.asList(Status.INSTALLING);
	...
	public void validate(InstallableInstance instance, Status nextStatus)
		...
		if (transitableStates == null
				|| !transitableStates.contains(nextStatus)) {
