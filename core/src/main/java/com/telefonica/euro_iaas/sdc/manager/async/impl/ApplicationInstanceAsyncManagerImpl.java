...
public class ApplicationInstanceAsyncManagerImpl implements ApplicationInstanceAsyncManager {
    ...
    private static Logger LOGGER = Logger.getLogger(ApplicationInstanceAsyncManagerImpl.class.getName());
    ...
    @Async
    @Override
    public void install(VM vm, String vdc, EnvironmentInstance environmentInstance, ApplicationRelease application,
            List<Attribute> configuration, Task task, String callback) {
        try {
            ApplicationInstance applicationInstance = applicationInstanceManager.install(vm, vdc, environmentInstance,
                    application, configuration);
            updateSuccessTask(task, applicationInstance);
            LOGGER.info("Application " + application.getApplication().getName() + '-' + application.getVersion()
                    + " installed successfully");

        } catch (NodeExecutionException e) {
            String errorMsg = "The application " + application.getApplication().getName() + "-"
                    + application.getVersion() + " can not be installed in" + vm + "due to a problem when"
                    + " executing in node";
            updateErrorTaskOnInstall(application, vm, task, errorMsg, e);
        } catch (IncompatibleProductsException e) {
            String errorMsg = "The application " + application.getApplication().getName() + "-"
                    + application.getVersion() + " can not be installed in" + vm + "due " + e.getProducts().size()
                    + " selected products" + "to install on are incompatible";
            updateErrorTaskOnInstall(application, vm, task, errorMsg, e);
        } catch (AlreadyInstalledException e) {
            String errorMsg = "The application " + application.getApplication().getName() + "-"
                    + application.getVersion() + " is already installed in" + vm;
            updateErrorTaskOnInstall(application, vm, task, errorMsg, e);
        } catch (NotInstalledProductsException e) {
            String errorMsg = "The application " + application.getApplication().getName() + "-"
                    + application.getVersion() + " can not be installed in" + vm + "due " + e.getProducts().size()
                    + " selected products" + "to install on are not installed";
            updateErrorTaskOnInstall(application, vm, task, errorMsg, e);
        } catch (Throwable e) {
            String errorMsg = "The application " + application.getApplication().getName() + "-"
                    + application.getVersion() + " can not be installed in" + vm + "due to unexpected error.";
            updateErrorTaskOnInstall(application, vm, task, errorMsg, e);
        } finally {
    ...
    @Async
    @Override
    public void configure(ApplicationInstance applicationInstance, List<Attribute> configuration, Task task,
            String callback) {
        try {
            applicationInstanceManager.configure(applicationInstance, configuration);
            LOGGER.info("Application " + applicationInstance.getApplication().getApplication().getName() + "-"
                    + applicationInstance.getApplication().getVersion() + "  configured successfully");
            updateSuccessTask(task, applicationInstance);
        } catch (NodeExecutionException e) {
            updateErrorTask(applicationInstance, task, "The application " + applicationInstance.getId()
    ...
    @Async
    @Override
    public void upgrade(ApplicationInstance applicationInstance, ApplicationRelease newRelease, Task task,
            String callback) {
        try {
            applicationInstanceManager.upgrade(applicationInstance, newRelease);
            updateSuccessTask(task, applicationInstance);
            LOGGER.info("Application " + applicationInstance.getApplication().getApplication().getName() + "-"
                    + applicationInstance.getApplication().getVersion() + " upgraded successfully");
        } catch (NodeExecutionException e) {
            updateErrorTask(applicationInstance, task, "The application " + applicationInstance.getId()
                    + " can not be upgraded due to an error executing in node.", e);
        } catch (NotTransitableException e) {
            updateErrorTask(applicationInstance, task, "The application " + applicationInstance.getId()
                    + " can not be upgraded due to the new version is " + "incompatible with the installed version", e);
        } catch (IncompatibleProductsException e) {
            updateErrorTask(applicationInstance, task, "The application " + applicationInstance.getId()
                    + " can not be upgraded due to the new version is " + " incompatible with "
                    + e.getProducts().size() + " products where the application is installed on.", e);
        } catch (FSMViolationException e) {
            updateErrorTask(applicationInstance, task, "The application " + applicationInstance.getId()
    ...
    @Async
    @Override
    public void uninstall(ApplicationInstance applicationInstance, Task task, String callback) {
        try {
            applicationInstanceManager.uninstall(applicationInstance);
            updateSuccessTask(task, applicationInstance);
            LOGGER.info("Application " + applicationInstance.getApplication().getApplication().getName() + "-"
                    + applicationInstance.getApplication().getVersion() + " uninstalled successfully");
        } catch (FSMViolationException e) {
            updateErrorTask(applicationInstance, task, "The application " + applicationInstance.getId()
    ...
    private void updateSuccessTask(Task task, ApplicationInstance applicationInstance) {
        VM vm = applicationInstance.getEnvironmentInstance().getProductInstances().iterator().next().getVm();
        String aiResource = MessageFormat.format(propertiesProvider.getProperty(APPLICATION_INSTANCE_BASE_URL),
                applicationInstance.getId(), // the id
                vm.getHostname(), // the hostname
                vm.getDomain(), // the domain
                applicationInstance.getApplication().getApplication().getName(), applicationInstance.getVdc()); // the
                                                                                                                // product
     ...
     */
    private void updateErrorTask(ApplicationInstance applicationInstance, Task task, String message, Throwable t) {
        VM vm = applicationInstance.getEnvironmentInstance().getProductInstances().iterator().next().getVm();
        String aiResource = MessageFormat.format(propertiesProvider.getProperty(APPLICATION_INSTANCE_BASE_URL),
                applicationInstance.getId(), // the id
                vm.getHostname(), // the hostname
    ...
    private ApplicationInstance getInstalledApplication(ApplicationRelease application, VM vm) {
     ...
     */
    private void updateErrorTaskOnInstall(ApplicationRelease application, VM vm, Task task, String errorMsg, Throwable e) {
     ...
     *            the applicationInstanceManager to set
     */
