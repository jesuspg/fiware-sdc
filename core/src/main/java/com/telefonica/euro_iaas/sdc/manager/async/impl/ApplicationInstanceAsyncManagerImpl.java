...
public class ApplicationInstanceAsyncManagerImpl implements
	...
	private static Logger LOGGER = Logger
			.getLogger(ApplicationInstanceAsyncManagerImpl.class.getName());
	...
	@Async
	@Override
	public void install(VM vm, String vdc,
		...
		try {
			ApplicationInstance applicationInstance = applicationInstanceManager
					.install(vm, vdc, environmentInstance, application,
							configuration);
			...
			String errorMsg = "The application "
					...
					+ application.getVersion() + " can not be installed in"
					+ vm + "due to a problem when" + " executing in node";
			updateErrorTaskOnInstall(application, vm, task, errorMsg, e);
		} catch (IncompatibleProductsException e) {
			String errorMsg = "The application "
					+ application.getApplication().getName() + "-"
					+ application.getVersion() + " can not be installed in"
					+ vm + "due " + e.getProducts().size()
					+ " selected products" + "to install on are incompatible";
			updateErrorTaskOnInstall(application, vm, task, errorMsg, e);
		} catch (AlreadyInstalledException e) {
			String errorMsg = "The application "
					+ application.getApplication().getName() + "-"
					+ application.getVersion() + " is already installed in"
					+ vm;
			updateErrorTaskOnInstall(application, vm, task, errorMsg, e);
	...
	public void configure(ApplicationInstance applicationInstance,
		...
		try {
			applicationInstanceManager.configure(applicationInstance,
					configuration);
			LOGGER.info("Application "
					+ applicationInstance.getApplication().getApplication()
							.getName() + "-"
			...
			updateErrorTask(
					...
					"The application "
							...
							+ " can not be configured due to an error executing in node.",
					e);
		} catch (FSMViolationException e) {
			updateErrorTask(applicationInstance, task, "The application "
					+ applicationInstance.getId()
	...
	public void upgrade(ApplicationInstance applicationInstance,
		...
		try {
			...
			updateErrorTask(
					...
					"The application "
							+ applicationInstance.getId()
							+ " can not be upgraded due to an error executing in node.",
					e);
	...
	private void updateSuccessTask(Task task,
		...
		VM vm = applicationInstance.getEnvironmentInstance()
				.getProductInstances().iterator().next().getVm();
		String aiResource = MessageFormat
				.format(propertiesProvider
						.getProperty(APPLICATION_INSTANCE_BASE_URL),
						applicationInstance.getId(), // the id
						vm.getHostname(), // the hostname
						vm.getDomain(), // the domain
						applicationInstance.getApplication().getApplication()
								.getName(), applicationInstance.getVdc()); // the
																			// product
	 ...
	 */
	private void updateErrorTask(ApplicationInstance applicationInstance,
	 ...
	 */
	private void updateErrorTaskOnInstall(ApplicationRelease application,
			VM vm, Task task, String errorMsg, Throwable e) {
