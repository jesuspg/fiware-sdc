...
public class ChefClientAsyncManagerImpl implements ChefClientAsyncManager {
	...
	private static Logger LOGGER = Logger
			.getLogger(ChefClientAsyncManagerImpl.class.getName());
	...
	/* (non-Javadoc)
	 * @see com.telefonica.euro_iaas.sdc.manager.async.ChefNodeAsyncManager#chefNodeDelete(java.lang.String, com.telefonica.euro_iaas.sdc.model.Task, java.lang.String)
	 */
	public void chefClientDelete(String vdc, String chefClientname, Task task, String callback) {
		try {
			...
			LOGGER.info("ChefClient   "
					+ chefClientname + " deleted from Chef Server successfully");
		} catch (ChefClientExecutionException e) {
			updateErrorTask(vdc, chefClientname,
					task,
					"The ChefClient "
							+ chefClientname
			...
			updateErrorTask(vdc, chefClientname, task, "The ChefClient "
					+ chefClientname
	...
	private void updateSuccessTask(Task task, String vdc, String chefClientname) {
		String piResource = MessageFormat.format(propertiesProvider
				.getProperty(CHEF_NODE_BASE_URL), vdc, chefClientname); // the product
	 ...
	 */	
	private void updateErrorTask(String vdc, String chefClientname, Task task,
			String message, Throwable t) {
		String piResource = MessageFormat.format(
				propertiesProvider.getProperty(CHEF_NODE_BASE_URL),
				vdc, chefClientname); // the product
		task.setResult(new TaskReference(piResource));
	...
	private void updateErrorTask(Task task, String message, Throwable t) {
		...
		taskManager.updateTask(task);
		LOGGER
				.info("An error occurs while deleting a node fromChef Server. See task "
