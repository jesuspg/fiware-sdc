...
public class EnvironmentManagerImpl implements EnvironmentManager {
	...
	public Environment load(String name) throws EnvironmentNotFoundException {
		...
		try {
			...
			String environmentNotFoundException = " The Environment " + name
					+ " DOES NOT EXIST ";
	...
	public Environment insert(Environment environment)
			throws ProductNotFoundException, ProductReleaseNotFoundException,
			AlreadyExistsEnvironmentException, InvalidEnvironmentException {
	...
	public void delete(String name) throws EnvironmentNotFoundException,
	...
	public Environment update(Environment environment)
			throws EnvironmentNotFoundException, InvalidEnvironmentException,
			EnvironmentNotFoundException, ProductReleaseNotFoundException,
			ProductNotFoundException {

		return updateEnvironmentBBDD(environment);
	}

	private Environment insertEnvironmentBBDD(Environment environment)
			throws ProductNotFoundException, AlreadyExistsEnvironmentException,
		...
		for (int i = 0; i < productReleases.size(); i++) {
			// Product
			try {
				product = productDao.load(productReleases.get(i).getProduct()
						.getName());
			} catch (EntityNotFoundException e) {
				String productNotFoundException = " The Product "
						+ productReleases.get(i).getProduct().getName()
						+ " NOT FOUND ";
			...
			try {
				productRelease = productReleaseDao.load(product,
						productReleases.get(i).getVersion());
			} catch (EntityNotFoundException e) {
				String productReleaseNotFoundException = " The Product Release "
						+ productReleases.get(i).getProduct().getName()
						+ "-"
						+ productReleases.get(i).getVersion() + " NOT FOUND ";
		...
		try {
			...
			try {
				...
				LOGGER.log(Level.INFO, "eNVIORNMENT " + env.getName()
						+ " CREATED");
			} catch (InvalidEntityException e1) {
				String invalidEnvironmentException = " The Environment "
						+ envIn.getName() + " is INVALID ";
				LOGGER.log(Level.SEVERE, invalidEnvironmentException);
				throw new InvalidEnvironmentException(
						invalidEnvironmentException);
			} catch (AlreadyExistsEntityException e1) {
				String alreadyExistsEnvironmentException = " The Environment "
						+ envIn.getName() + " ALREADY EXISTS ";
				LOGGER.log(Level.SEVERE, alreadyExistsEnvironmentException);
				throw new AlreadyExistsEnvironmentException(
						alreadyExistsEnvironmentException);
			}
	...
	private Environment updateEnvironmentBBDD(Environment environment)
			throws EnvironmentNotFoundException, InvalidEnvironmentException,
		...
		LOGGER.log(Level.INFO, "Environment  Before Loading "
				+ environment.getName());

		for (int i = 0; i < environment.getProductReleases().size(); i++) {
			ProductRelease productRelease = environment.getProductReleases()
					.get(i);
			...
			try {
				...
				String productNotFoundException = " The Product "
						+ productRelease.getProduct().getName() + " NOT FOUND ";
				LOGGER.log(Level.SEVERE, productNotFoundException);
				throw new ProductNotFoundException(productNotFoundException);
			}

			try {
				existedProductRelease = productReleaseDao.load(existedProduct,
						productRelease.getVersion());
			} catch (EntityNotFoundException e) {
				String productReleaseNotFoundException = " The Product Release "
						+ productRelease.getProduct().getName()
						+ "-"
						+ productRelease.getVersion() + " NOT FOUND ";
				LOGGER.log(Level.SEVERE, productReleaseNotFoundException);
				throw new ProductReleaseNotFoundException(
						productReleaseNotFoundException);
			}
		...
		try {
			...
			String InvalidEnvironmentNotFoundException = " The Environment "
					+ environment.getName() + " IS NOT VALID ";
			LOGGER.log(Level.SEVERE, InvalidEnvironmentNotFoundException);
			throw new ProductReleaseNotFoundException(
					InvalidEnvironmentNotFoundException);

		}
		return environment;
	}

	private void deleteEnvironmentBBDD(String name)
			throws EnvironmentNotFoundException,
			ProductReleaseStillInstalledException,
			ProductReleaseInApplicationReleaseException {
