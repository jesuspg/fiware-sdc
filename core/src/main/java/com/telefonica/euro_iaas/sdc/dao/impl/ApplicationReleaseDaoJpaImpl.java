...
public class ApplicationReleaseDaoJpaImpl extends
		AbstractBaseDao<ApplicationRelease, Long> implements
	...
	public List<ApplicationRelease> findByCriteria(
		...
		if (criteria.getApplication() != null) {
			baseCriteria.add(Restrictions.eq("application", criteria
					.getApplication()));
		}

		List<ApplicationRelease> applications = setOptionalPagination(criteria,
				baseCriteria).list();

		if (criteria.getProductRelease() != null) {
			applications = filterByProduct(applications, criteria
					.getProductRelease());
		}
		return applications;
	}
	...
	public ApplicationRelease load(Application application, String version)
		...
		ApplicationRelease release = (ApplicationRelease) baseCriteria
				.uniqueResult();
		if (release == null) {
			String[] keys = { "application", "version" };
			Object[] values = { application, version };
			throw new EntityNotFoundException(ApplicationRelease.class, keys,
					values);
		}
		return release;
	}
	...
	private List<ApplicationRelease> filterByProduct(
		...
		for (ApplicationRelease application : applications) {
			if (application.getEnvironment().getProductReleases().contains(
					product)) {
