...
public class ApplicationInstanceDaoJpaImplTest extends AbstractJpaDaoTest {
	...
	public void createElements() throws Exception {
		...
		ProductRelease tomcat7 = new ProductRelease("7", "Tomcat server 7",
				null, tomcat, supportedSSOO, null);
		...
		ProductRelease tomcat6 = new ProductRelease("6", "Tomcat server 6",
				null, tomcat, supportedSSOO, null);
		...
		ProductRelease tomcat5 = new ProductRelease("5.5", "Tomcat server 5.5",
				null, tomcat, supportedSSOO, Arrays.asList(tomcat6));
		...
		Environment tomcat567Env = new Environment(Arrays.asList(tomcat5,
				tomcat6, tomcat7));
		...
		ProductInstance tomcat7Instance = new ProductInstance(tomcat7,
				Status.INSTALLED, new VM("fqn2", "ip2", "host2", "domain2"),
				"vdc");
		tomcat7Instance = productInstanceDao.create(tomcat7Instance);

		EnvironmentInstance envInstancet6 = new EnvironmentInstance(tomcat6Env,
				Arrays.asList(tomcat6Instance));
		envInstancet6 = environmentInstanceDao.create(envInstancet6);

		EnvironmentInstance envInstancet7 = new EnvironmentInstance(tomcat7Env,
				Arrays.asList(tomcat7Instance));
		envInstancet7 = environmentInstanceDao.create(envInstancet7);

		sdc030Instance = new ApplicationInstance(sdc030, envInstancet6,
				Status.INSTALLING, tomcat6Instance.getVm(), "vdc");
		sdc030Instance = applicationInstanceDao.create(sdc030Instance);

		sdc040Instance = new ApplicationInstance(sdc040, envInstancet7,
				Status.ERROR, tomcat7Instance.getVm(), "vdc");
		sdc040Instance = applicationInstanceDao.create(sdc040Instance);

		Application wordpress = new Application("wordpress",
				"this application", "php");
		wordpress = applicationDao.create(wordpress);

		ApplicationRelease wp5 = new ApplicationRelease("5",
				"Add configuration functionallity", null, wordpress,
				tomcat567Env, null);
		wp5 = applicationReleaseDao.create(wp5);

		EnvironmentInstance envInstancet567 = new EnvironmentInstance(
				tomcat567Env, Arrays.asList(tomcat5Instance, tomcat6Instance,
						tomcat7Instance));
		envInstancet7 = environmentInstanceDao.create(envInstancet567);

		ApplicationInstance wp5instance = new ApplicationInstance(wp5,
				envInstancet567, Status.UNINSTALLED, tomcat7Instance.getVm(),
				"vdc");
	...
	public void testFindByProduct() throws Exception {
		...
		List<ApplicationInstance> instances = applicationInstanceDao
	...
	public void testFindByStatus() throws Exception {
		...
		criteria
				.setStatus(Arrays.asList(Status.INSTALLING, Status.UNINSTALLED));
		instances = applicationInstanceDao.findByCriteria(criteria);
