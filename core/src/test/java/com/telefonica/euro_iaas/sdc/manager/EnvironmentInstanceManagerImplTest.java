...
public class EnvironmentInstanceManagerImplTest {
	...
	public void setUp() throws Exception {
		...
		pr1 = new ProductRelease("version1", "releaseNotes1", null, p1, null,
				null);
		pr2 = new ProductRelease("version2", "releaseNotes2", null, p2, null,
				null);

		environment = new Environment(Arrays.asList(pr1, pr2));

		pi1 = new ProductInstance(pr1, ProductInstance.Status.INSTALLED, vm,
				"vdc");
		pi2 = new ProductInstance(pr2, ProductInstance.Status.INSTALLED, vm,
				"vdc");
		...
		when(
				environmentInstanceDao.create(Mockito
						.any(EnvironmentInstance.class))).thenReturn(
				envInstance);
		when(environmentInstanceDao.load(Mockito.any(Long.class))).thenReturn(
				envInstance);

		environmentDao = mock(EnvironmentDao.class);
		when(environmentDao.create(Mockito.any(Environment.class))).thenReturn(
				environment);
		when(environmentDao.update(Mockito.any(Environment.class))).thenReturn(
				environment);
		when(environmentDao.load(Mockito.any(String.class))).thenReturn(
				environment);
	...
	public void testInsertWhenEverythingIsOk() throws Exception {
		...
		// execution
		EnvironmentInstance installedEnvInstance = envInstanceManager
				.insert(new EnvironmentInstance(environment, products));

		// make assertions
		Assert.assertEquals(installedEnvInstance, envInstance);
		Assert.assertEquals(installedEnvInstance.getEnvironment(), environment);
		Assert.assertEquals(installedEnvInstance.getProductInstances(),
				products);
