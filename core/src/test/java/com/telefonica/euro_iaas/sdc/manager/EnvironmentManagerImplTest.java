...
public class EnvironmentManagerImplTest {
	...
	public void setUp() throws Exception {
		...
		when(environmentDao.create(any(Environment.class))).thenReturn(
				expectedEnvironment);
		when(environmentDao.update(any(Environment.class))).thenReturn(
				expectedEnvironment);
		when(environmentDao.load(any(String.class))).thenReturn(
				expectedEnvironment);
	...
	public void testInsertWhenEverythingIsOk() throws Exception {
		...
		List<ProductRelease> productReleasesInside = Arrays
				.asList(productRelease);
		Environment installedEnvironment = envManager.insert(new Environment(
				productReleasesInside));
		// make verifications
	...
	public void testUpdateWhenEverithingIsOk() throws Exception {
		...
		List<ProductRelease> productReleasesInside = Arrays
				.asList(productRelease);
		Environment updatedEnvironment = envManager.update(new Environment(
				productReleasesInside));
