...
public class SDCClientUtilsImplTest extends TestCase {
    ...
    public void setUp() throws Exception { 	
        ...
        when(nodeCommandDao.findByCriteria(new NodeCommandSearchCriteria(any(OS.class))))
        .thenReturn(nodeCommands);

        propertiesProvider = mock(SystemPropertiesProvider.class);
        when(propertiesProvider.getProperty(CHEF_CLIENT_URL_TEMPLATE)).thenReturn(
                "http://{0}:9990/sdc-client");
        ...
        //doReturn(clientResponse).when(webResource).accept(MediaType.APPLICATION_XML).put(ClientResponse.class, anyList());;
        
        //doReturn().when(webResource).accept((MediaType) anyVararg()).type(anyString()).entity(anyList()).put(anyList());
    }
    ...
    public void testSetNodeCommandOk(){
        ...
        try {
        	sdcClientUtilsImpl.setNodeCommands(host);
