...
public class RsaSignerTest {
	...
	public void testSign() throws Exception {
		Assert.assertEquals(SIGNED, new RSASignerImpl().sign(BODY, new File(
				this.getClass().getResource("/private.pem").toURI())));
	}

	@Test
	public void testUnsign() throws Exception {
		Assert.assertEquals(BODY, new RSASignerImpl().unsign(SIGNED, new File(
				this.getClass().getResource("/public.pem").toURI())));
	}
