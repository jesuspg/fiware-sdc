...
public class ApplicationInstanceManagerChefImplTest {
	...
	public void setUp() throws Exception {
		...
		when(
				recipeNamingGenerator
						.getInstallRecipe(any(ApplicationInstance.class)))
				.thenReturn("war::app-p1-p2");
		...
		appRelease = new ApplicationRelease("version", "releaseNotes", null,
		...
		when(
				applicationInstanceDao.create(Mockito
						.any(ApplicationInstance.class))).thenReturn(
				applicationInstance);
		when(
				applicationInstanceDao.update(Mockito
						.any(ApplicationInstance.class))).thenReturn(
				applicationInstance);
		when(
				applicationInstanceDao.findUniqueByCriteria(Mockito
		...
		when(chefNodeDao.updateNode((ChefNode) anyObject())).thenReturn(
				new ChefNode());
	...
	public void testInstallWhenEverythingIsOk() throws Exception {
		...
		// execution
		ApplicationInstance installedApp = manager.install(vm, "vdc",
				envInstance, appRelease, new ArrayList<Attribute>());
		// make assertions
