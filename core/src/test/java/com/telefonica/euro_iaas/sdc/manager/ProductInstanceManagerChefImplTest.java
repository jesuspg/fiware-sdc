...
public class ProductInstanceManagerChefImplTest extends TestCase {
	...
	public void testInstallWhenEverithingIsOk() throws Exception {
        ...
        Mockito.doThrow(new EntityNotFoundException(ProductInstance.class, "test", expectedProduct)).when(productInstanceDao).load(any(String.class)); 
		
		expectedProduct = new ProductInstance(productRelease, Status.INSTALLED,
				host, "vdc");
		when(productInstanceDao.create(any(ProductInstance.class))).thenReturn(
				expectedProduct);
		ProductInstance installedProduct = manager.install(host, "vdc",
				productRelease, new ArrayList<Attribute>());
		// make verifications
...
//		verify(chefNodeDao, times(1)).updateNode((ChefNode) anyObject());
	...
	public void testUnInstallAndInstall() throws Exception {
		...
		expectedProduct = manager.install(host, "vdc",
				productRelease, new ArrayList<Attribute>());
	...
	public void testInstallProductNotValidStatus() throws Exception {
		...
		expectedProduct = new ProductInstance(productRelease, Status.UNINSTALLING,
				host, "vdc");
