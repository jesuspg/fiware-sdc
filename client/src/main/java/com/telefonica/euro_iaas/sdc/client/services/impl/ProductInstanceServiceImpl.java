...
public class ProductInstanceServiceImpl extends AbstractInstallableService
	...
	public Task installArtifact(String vdc, String product,
			com.telefonica.euro_iaas.sdc.model.Artifact artifact,
			String callback) {
		String url = getBaseHost() + MessageFormat.format(

		ClientConstants.INSTALL_ARTEFACT_INSTANCE_PATH, vdc, product);
		WebResource wr = getClient().resource(url);
		Builder builder = wr.accept(getType()).type(getType()).entity(artifact);
		builder = addCallback(builder, callback);
		return builder.post(Task.class);
	}
	
	public Task uninstallArtifact(String vdc, String productInstanceId,
			Artifact artefact, String callback) {
		String url = getBaseHost() + MessageFormat.format(
	...
	public ProductInstance load(String vdc, String name)
		...
		String url = getBaseHost()
				+ MessageFormat.format(ClientConstants.PRODUCT_INSTANCE_PATH,
						vdc, name);
