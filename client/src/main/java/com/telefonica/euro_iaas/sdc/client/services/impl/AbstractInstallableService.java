...
public abstract class AbstractInstallableService extends AbstractBaseService {
	...
	/**
	 * See
	 * {@link ApplicationInstanceService#upgrade(String, Long, String, String)}
	...
	public Task upgrade(String vdc, String name, String version, String callback) {
		String url = getBaseHost()
				+ MessageFormat.format(upgradePath, vdc, name, version);
	...
	/**
	 * See
	 * {@link ApplicationInstanceService#configure(String, Long, String, Attributes)}
	 * or
	...
	public Task configure(String vdc, String name, String callback,
		...
		String url = getBaseHost()
				+ MessageFormat.format(configPath, vdc, name);
		WebResource wr = getClient().resource(url);
	...
	public Task uninstall(String vdc, String name, String callback) {
		String url = getBaseHost()
				+ MessageFormat.format(uninstallPath, vdc, name);
		WebResource wr = getClient().resource(url);
		Builder builder = wr.accept(getType());
		builder = addCallback(builder, callback);
