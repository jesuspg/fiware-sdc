...
public class ProductServiceImpl extends AbstractBaseService implements
	 ...
	 */
	@Override
	public ProductRelease add(ProductReleaseDto releaseDto,
	 ...
	 */
	@Override
	public ProductRelease update(ProductReleaseDto releaseDto,
			InputStream cookbook, InputStream files) {
		try {
			...
			String url = getBaseHost()
					+ MessageFormat.format(
							ClientConstants.PRODUCT_RELEASE_PATH, releaseDto
									.getProductName(), releaseDto.getVersion());
			WebResource wr = getClient().resource(url);
			return wr.accept(getType()).type("multipart/mixed").entity(payload)
	...
	public void delete(String pname, String version) {
		...
		String url = getBaseHost()
				+ MessageFormat.format(ClientConstants.PRODUCT_RELEASE_PATH,
						pname, version);
		;
		WebResource wr = getClient().resource(url);
	...
	public List<ProductRelease> findAll(Integer page, Integer pageSize,
		...
		if (StringUtils.isEmpty(productName)) {
			...
			url = getBaseHost()
					+ MessageFormat.format(
							ClientConstants.BASE_PRODUCT_RELEASE_PATH,
							productName);
		}
		WebResource wr = getClient().resource(url);
