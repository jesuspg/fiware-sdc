...
public class ApplicationServiceImpl extends AbstractBaseService implements ApplicationService {
    ...
    public ApplicationServiceImpl(Client client, String baseUrl, String mediaType) {
        setBaseHost(baseUrl);
        setType(mediaType);
        setClient(client);
    }
     ...
     */
    @Override
    public ApplicationRelease add(ApplicationReleaseDto releaseDto, InputStream cookbook, InputStream files) {
        try {
            ...
            return wr.accept(getType()).type("multipart/mixed").entity(payload).post(ApplicationRelease.class);
     ...
     */
    @Override
    public ApplicationRelease update(ApplicationReleaseDto releaseDto, InputStream cookbook, InputStream files) {
        try {
            ...
            String url = getBaseHost()
                    + MessageFormat.format(ClientConstants.APPLICATION_RELEASE_PATH, releaseDto.getApplicationName(),
                            releaseDto.getVersion());
            WebResource wr = getClient().resource(url);
            return wr.accept(getType()).type("multipart/mixed").entity(payload).put(ApplicationRelease.class);
        } catch (FileNotFoundException e) {
            throw new RuntimeException(e);
        } catch (IOException e) {
            throw new RuntimeException(e);
    ...
    public void delete(String appname, String version) {
        ...
        String url = getBaseHost() + MessageFormat.format(ClientConstants.APPLICATION_RELEASE_PATH, appname, version);
        ;
        WebResource wr = getClient().resource(url);
     ...
     */
    @Override
    public ApplicationRelease load(String application, String version) throws ResourceNotFoundException {
        String url = getBaseHost()
                + MessageFormat.format(ClientConstants.APPLICATION_RELEASE_PATH, application, version);
        WebResource wr = getClient().resource(url);
        try {
    ...
    @Override
    public List<ApplicationRelease> findAll(Integer page, Integer pageSize, String orderBy, String orderType,
            String applicationName) {
        String url;
        if (StringUtils.isEmpty(applicationName)) {
            url = getBaseHost() + ClientConstants.ALL_APPLICATION_RELEASE_PATH;
        } else {
            url = getBaseHost() + MessageFormat.format(ClientConstants.BASE_APPLICATION_RELEASE_PATH, applicationName);
        }
        WebResource wr = getClient().resource(url);
        ...
        return wr.queryParams(searchParams).accept(getType()).get(ApplicationReleases.class);
    }
}
