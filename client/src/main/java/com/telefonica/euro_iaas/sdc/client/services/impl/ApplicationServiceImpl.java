...
public class ApplicationServiceImpl extends AbstractBaseService implements
    ...
    public ApplicationRelease update(ApplicationReleaseDto releaseDto,
        ...
        String url = getBaseHost() + MessageFormat.format(
        		...
        		releaseDto.getApplicationName(), releaseDto.getVersion());
        WebResource wr = getClient().resource(url);
        return wr.accept(getType()).type("multipart/mixed").entity(payload)
    ...
    public void delete(String appname, String version) {
        ...
        String url = getBaseHost() + MessageFormat.format(
                        ClientConstants.APPLICATION_RELEASE_PATH, appname, version);;
        WebResource wr = getClient().resource(url);
    ...
    public ApplicationRelease load(String application, String version)
        ...
        String url = getBaseHost() + MessageFormat.format(
                ClientConstants.APPLICATION_RELEASE_PATH, application, version);
        WebResource wr = getClient().resource(url);
        try {
    ...
    public List<ApplicationRelease> findAll(Integer page, Integer pageSize,
        ...
        if (StringUtils.isEmpty(applicationName)) {
            url = getBaseHost() +
                    ClientConstants.ALL_APPLICATION_RELEASE_PATH;
        } else {
            url = getBaseHost() + MessageFormat.format(
                    ClientConstants.BASE_APPLICATION_RELEASE_PATH, applicationName);
        }
        WebResource wr = getClient().resource(url);
        ...
        return wr.queryParams(searchParams)
                .accept(getType()).get(ApplicationReleases.class);
    }
}
