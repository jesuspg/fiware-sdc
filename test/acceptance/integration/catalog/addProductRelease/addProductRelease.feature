...
Feature: Add a new product release in the catalogue
    ...
    Scenario: add a new release with non-existent product in the catalogue
        ...
        When I add a new release "1.2.3" with description "version 1.2.3 for test" associated to product "Product_test_0001_error" in the catalog with "xml" content in the response
    ...
    Scenario Outline: try to add a new product release with name errors into of the catalog
        Given The "Product_test_0001" has been created and the sdc is up and properly configured
         When I add a new release "<version>" with description "<error>" associated to product "Product_test_0001" in the catalog with "xml" content in the response
    ...
    Scenario: launch a Not acceptable error when I add a new product release into the catalogue with Accept header invalid
        Given The "Product_test_0001" has been created and the sdc is up and properly configured
        When  I add a new release "1.2.3" with description "version 1.2.3 for test" associated to product "Product_test_0001" in the catalog with "fgfg" content in the response
