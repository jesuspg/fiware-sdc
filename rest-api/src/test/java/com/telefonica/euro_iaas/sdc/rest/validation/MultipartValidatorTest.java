...
public class MultipartValidatorTest extends ValidatorUtils {
	...
	public void testValidateMultipartSize() throws Exception {
		MultiPart multiPart = new MultiPart().bodyPart(new BodyPart("OK",
	...
	public void testValidateMultipartFirstEntityPart() throws Exception {
		...
		MultiPart multiPart = new MultiPart().bodyPart(
				new BodyPart(st, MediaType.APPLICATION_JSON_TYPE))
				.bodyPart(
						new BodyPart(productReleaseDto,
								MediaType.APPLICATION_JSON_TYPE)).bodyPart(
						new BodyPart(productReleaseDto,
								MediaType.APPLICATION_JSON_TYPE));

		System.out.println("Clase of type " + productReleaseDto.getClass()
				+ " " + productReleaseDto.getProductName());
		try {
			multipartValidator.validateMultipart(multiPart, productReleaseDto
					.getClass());
			Assert.fail();
		} catch (InvalidMultiPartRequestException e) {
			// Expected Exception
	...
	public void testValidateMultipartSecondEntityPart() throws Exception {
		...
		MultiPart multiPart = new MultiPart()
				.bodyPart(
						...
						new BodyPart("OK", MediaType.APPLICATION_JSON_TYPE))
				.bodyPart(
						new BodyPart(productReleaseDto,
								MediaType.APPLICATION_JSON_TYPE));
