...
public class ApplicationResourceValidatorImplTest extends ValidatorUtils {
	...
	public void setUp() throws Exception {
		...
		applicationReleaseDto
				.setApplicationDescription("yumabcd 0.1.1 description");
		...
		List<ProductRelease> supporteProducts = Arrays.asList(productRelease);
		...
		for (int i = 0; i < supporteProducts.size(); i++) {
			...
			productReleaseDto.setProductName(supporteProducts.get(i)
					.getProduct().getName());
			productReleaseDto.setProductDescription(supporteProducts.get(i)
					.getProduct().getDescription());
			productReleaseDto.setPrivateAttributes(supporteProducts.get(i)
					.getPrivateAttributes());
			productReleaseDto.setReleaseNotes(supporteProducts.get(i)
					.getReleaseNotes());
			productReleaseDto.setSupportedOS(supporteProducts.get(i)
					.getSupportedOOSS());
			productReleaseDto.setTransitableReleases(supporteProducts.get(i)
					.getTransitableReleases());

			productReleaseDtos.add(productReleaseDto);
		}

		applicationReleaseDto.setEnvironmentDto(new EnvironmentDto(
				productReleaseDtos));
	...
	public void testUpdateValidateKO() throws Exception {
		/*
		 * File recipes = createTempFile ("recipes"); File installable =
		 * createTempFile ("installable");
		 * 
		 * byte[] bytesRecipes = getByteFromFile(recipes); byte[]
		 * bytesInstallable = getByteFromFile (installable);
		 * 
		 * deleteFile(recipes); deleteFile(installable);
		 * 
		 * System.out.println("AplictionreleaseDto " +
		 * applicationReleaseDto.getApplicationName()); MultiPart multiPart =
		 * new MultiPart().bodyPart(new BodyPart(
		 * applicationReleaseDto,MediaType.APPLICATION_XML_TYPE)) .bodyPart(new
		 * BodyPart(bytesRecipes, MediaType.APPLICATION_OCTET_STREAM_TYPE))
	...
	public void testUpdateValidateOK() throws Exception {
		...
		MultiPart multiPart = new MultiPart().bodyPart(
				new BodyPart(applicationReleaseDto,
						MediaType.APPLICATION_JSON_TYPE)).bodyPart(
				new BodyPart(bytesRecipes,
						MediaType.APPLICATION_OCTET_STREAM_TYPE)).bodyPart(
				new BodyPart(bytesInstallable,
						MediaType.APPLICATION_OCTET_STREAM_TYPE));
