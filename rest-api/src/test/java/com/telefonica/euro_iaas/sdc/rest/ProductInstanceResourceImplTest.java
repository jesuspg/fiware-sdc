...
public class ProductInstanceResourceImplTest {
...

	{
		...
		productInstanceResource
				.setProductInstanceAsyncManager(productInstanceAsyncManager);
		...
		when(
				productInstanceAsyncManager.load(any(String.class),
						any(String.class))).thenReturn(productIns);
		when(
				productInstanceAsyncManager
						.findByCriteria(any(ProductInstanceSearchCriteria.class)))
				.thenReturn(lProductInstance);
		doNothing().when(productInstanceAsyncManager).install(any(VM.class),
				any(String.class), any(ProductRelease.class), any(List.class),
	...
	public void testInstallProduct() throws Exception {
		...
		String callback = "";
		Task task = productInstanceResource.install(VDC, productInstance,
				callback);
	...
	public void testListProduct() throws Exception {
		...
		List<ProductInstance> pProductInstance = productInstanceResource
				.findAll("hostname", "domain", "ip", "fqn", null, null, null,
						null, Status.INSTALLED, VDC, "Product::server");
