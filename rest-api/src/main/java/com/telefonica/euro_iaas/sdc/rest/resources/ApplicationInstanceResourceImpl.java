...
public class ApplicationInstanceResourceImpl implements
	...
	public Task install(String vdc, ApplicationInstanceDto application,
		...
		try {
			...
			Task task = createTask(MessageFormat.format(
					"Install application {0} in  VM {1}{2}", application
							.getApplicationName(), vm.getHostname(), vm
							.getDomain()), vdc);
			...
			if (products != null) {
				...
				for (ProductReleaseDto relDto : products) {
					...
					ProductRelease product = productManager.load(p, relDto
							.getVersion());
					...
					try {
						...
						environment_name = environment_name
								+ pInstance.getProductRelease().getProduct()
										.getName()
								+ pInstance.getProductRelease().getVersion()
								+ "_";
				...
				if (!notFoundProducts.isEmpty()) {
					...
					error
							.setVenodrSpecificErrorCode("NotInstalledProductsException");

					task.setError(error);
					return taskManager.updateTask(task);
				}
			}
			Application app = applicationManager.load(application
					.getApplicationName());
			ApplicationRelease release = applicationManager.load(app,
			...
			EnvironmentInstanceDto environmentInstanceDto = application
					.getEnvironmentInstanceDto();
			...
			EnvironmentInstance environmentInstance = environmentInstanceManager
					.load(new EnvironmentInstance(environment, productList)
							.getId());

			applicationInstanceAsyncManager.install(vm, vdc,
					environmentInstance, release, attributes, task, callback);
	...
	public Task uninstall(String vdc, String name, String callback) {
		...
		VM vm = app.getEnvironmentInstance().getProductInstances().iterator()
				.next().getVm();
		Task task = createTask(MessageFormat.format(
				"Uninstall application {0} in  VM {1}{2}", app.getApplication()
						.getApplication().getName(), vm.getHostname(), vm
						.getDomain()), vdc);
	 ...
	 */
	/*
	 * @Override public ApplicationInstance load(Long id) { try { return
	 * applicationInstanceAsyncManager.load(id); } catch
	 * (EntityNotFoundException e) { throw new WebApplicationException(e, 404);
	 * } }
	 */
	/**
	 * {@inheritDoc}
	...
	public ApplicationInstance load(String name) {
		/*
		 * try { return applicationInstanceAsyncManager.load(name); } catch
		 * (EntityNotFoundException e) { throw new WebApplicationException(e,
		 * 404); }
		 */
		return null;
	}
	...
	public List<ApplicationInstance> findAll(String hostname, String domain,
		...
		Boolean validHost = (!StringUtils.isEmpty(ip) && !StringUtils
				.isEmpty(fqn))
				|| (!StringUtils.isEmpty(domain) && !StringUtils
						.isEmpty(hostname));
		if (validHost) {
			VM host = new VM(fqn != null ? fqn : "", ip != null ? ip : "",
					hostname != null ? hostname : "", domain != null ? domain
							: "");
			criteria.setVm(host);

			/*
			 * if (productName != null && productVersion != null){
			 * ProductInstance productInstance = new ProductInstance( new
			 * ProductRelease ( productVersion, null, null, new Product(
			 * productName, null), null, null), null, host, null);
	...
	public Task configure(String vdc, String name, String callback,
		...
		VM vm = application.getEnvironmentInstance().getProductInstances()
				.iterator().next().getVm();
		Task task = createTask(MessageFormat.format(
				"Configure application {0} in  VM {1}{2}", application
						.getApplication().getApplication().getName(), vm
						.getHostname(), vm.getDomain()), vdc);
		applicationInstanceAsyncManager.configure(application, arguments, task,
				callback);
		return task;
	}
	...
	public Task upgrade(String vdc, String name, String version, String callback) {
		 ...
		 * application.getEnvironmentInstance().getProductInstances()
		 * .iterator().next().getVm(); ApplicationRelease release =
		 * applicationManager.load(
		 * application.getApplication().getApplication(), version); Task task =
		 * createTask(MessageFormat.format("Upgrade application" +
		 * " {0} in  VM {1}{2} from version {3} to {4}",
		 * application.getApplication().getApplication().getName(),
		 * vm.getHostname(), vm.getDomain(), application.getApplication()
		 ...
		 * 
		 * applicationInstanceAsyncManager.upgrade(application, release, task,
		 * callback); return task; } catch (EntityNotFoundException e) { throw
		 * new SdcRuntimeException(e); }
		 */
		return null;
	}
}
