...
public class ProductInstanceResourceImpl implements ProductInstanceResource {
	...
	private Task uninstall(ProductInstance productInstance, String vdc,
			String callback) {
		Task task = createTask(MessageFormat.format(
				"Uninstall product {0} in  VM {1}{2}", productInstance
	...
	private Task upgrade(String vdc, ProductInstance productInstance,
		...
		try {
			...
			ProductRelease newRelease = productManager.load(productInstance
					.getProductRelease().getProduct(), version);
			...
			productInstanceAsyncManager.upgrade(productInstance, newRelease,
					task, callback);
			return task;
		} catch (EntityNotFoundException e) {
			throw new SdcRuntimeException(e);
	...
	public Task configure(String vdc, ProductInstance productInstance,
			Attributes arguments, String callback) {
		...
		Task task = createTask(MessageFormat.format(
				...
				productInstance.getVm().getHostname(), productInstance.getVm()
						.getDomain()), vdc);
		productInstanceAsyncManager.configure(productInstance, arguments, task,
				callback);
		return task;
	}
